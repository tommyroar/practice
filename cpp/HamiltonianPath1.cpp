...
class Graph {
    ...
    public:
        ...
        bool hamiltonPathUtil (vector <int> &path, vector <bool> &visited, int vertex);
...
bool Graph::vertexSafe (vector <int> path, int from, int to) {
    ...
    for (vector <int>::iterator it2 = adj[from].begin(); it2 != adj[from].end(); ++it2) {
        if ((*it2) == to) {
            cout << "vertex from " << from << " to " << to << " safe" << endl;
            return true;
        }
    }
...
bool Graph::hamiltonPathUtil (vector <int> &path, vector <bool> &visited, int vertex) {
    if (vertex == size) {
        cout << "last vertex; path complete" << endl;
        return true;
    }
    for (vector <int> :: iterator it = adj[vertex].begin();
        ...
        if (vertexSafe(path, vertex, dest)) {
            path[vertex] = dest;
            if (hamiltonPathUtil(path, visited, vertex+1)) {
                return true;
            }
            path[vertex] = -1;
        }
...
bool Graph::hamiltonPath (int vertex) {
    ...
    if ( hamiltonPathUtil(path, visited, 1) == false) {
